...
import {
  ...
  getTyporaAppData,
  getTyporaThemesDir,
  getTyporaBuiltinThemesDir,
  getTyporaBaseCss,
  getTyporaDraftsDir,
  getTyporaBackupsDir,
  getTyporaHistoryFile,
  getTyporaProfileFile,
  getTyporaUserConfigFile
} from "./config.js";

// Helper: Hex-encoded JSON file reader
export function readHexJson<T = any>(filePath: string): T | null {
  try {
...
export function writeHexJson(filePath: string, data: any): void {
 ...
 */
export async function getTyporaStatus(): Promise<TyporaStatus> {
  ...
  try {
    ...
    if (stdout && stdout.length > 0) {
      ...
      for (const p of parsed) {
        ...
        let type: TyporaProcessInfo["type"] = "unknown";
 ...
 */
export async function openInTypora(targetPath: string): Promise<{ success: boolean; path: string; message: string }> {
  const exePath = getTyporaExe();
 ...
 */
export async function closeTypora(pid?: number): Promise<{ success: boolean; message: string }> {
  if (pid) {
 ...
 */
export function getRecentDocuments(query?: string, limit = 20, verifyExists = true) {
  const histFile = getTyporaHistoryFile();
  const data = readHexJson<{ recentDocument?: Array<{ name: string; path: string; date: number }> }>(histFile);
...
export function getRecentFolders(limit = 20) {
  const histFile = getTyporaHistoryFile();
  const data = readHexJson<{ recentFolder?: Array<string | { path: string; date?: number }> }>(histFile);
 ...
 */
export function listDrafts(query?: string, limit = 30) {
 ...
 */
export function readDraft(filename: string, restoreToPath?: string) {
  const draftsDir = getTyporaDraftsDir();
  const fullPath = path.join(draftsDir, filename);
...
export function getPreferences() {
  ...
  const profile = readHexJson<Record<string, any>>(profileFile) || {};

  const userConfFile = getTyporaUserConfigFile();
  let userConf: Record<string, any> = {};
...
export function listThemes() {
  ...
  const activeProfile = readHexJson<Record<string, any>>(getTyporaProfileFile()) || {};
  const currentActiveTheme = activeProfile.theme || "github.css";
  const currentDarkTheme = activeProfile.darkTheme;

  const foundThemes = new Map<string, { name: string; file: string; type: "user" | "builtin"; isActive: boolean; isDarkActive: boolean }>();
 ...
 */
export function getThemeCss(themeName: string): { name: string; css: string; path: string } {
 ...
 */
export function installTheme(themeName: string, cssContent: string): { success: boolean; filePath: string } {
 ...
 */
export function setTheme(theme?: string, darkTheme?: string): { success: boolean; activeTheme: string; darkTheme?: string } {
  const profileFile = getTyporaProfileFile();
  const profile = readHexJson<Record<string, any>>(profileFile) || {};
...
export async function renderTyporaHtml(params: {
  ...
  theme?: string;
  embedCss?: boolean;
}): Promise<{ html: string; outputPath?: string; themeUsed: string; charCount: number }> {
  ...
  // Determine theme
  const chosenTheme = params.theme || (readHexJson<Record<string, any>>(getTyporaProfileFile())?.theme || "github.css");
  let themeCss = "";
  try {
...
export async function exportTyporaPdf(params: {
  ...
  outputPath: string;
  theme?: string;
}): Promise<{ pdfPath: string; sizeBytes: number; themeUsed: string }> {
